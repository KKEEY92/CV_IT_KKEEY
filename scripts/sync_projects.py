...
def main():
    ...
    print("Checking IT portfolio side project statuses from GitHub & GitLab...")
    updates = {}
    ...
    for proj_name, tag in updates.items():
        ...
        pattern = re.compile(rf"(name:\s*'{re.escape(proj_name)}'.*?desc:\s*\{{[^}}]*de:\s*').*?(')", re.DOTALL)
        def repl(match):
            old_text = match.group(0)
            if '●' in old_text:
                new_text = re.sub(r'v\d+\.\d+(\.\d+)?', f'v{clean_tag}', old_text)
                return new_text
            return old_text
